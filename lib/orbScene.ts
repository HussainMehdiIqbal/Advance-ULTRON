...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  // antialias + a pixel ratio of 2 roughly quadruples the pixels the GPU
  // has to shade every frame vs. a plain 1x canvas. Cheap on a modern
  // discrete GPU, but expensive on older integrated graphics (e.g. Intel
  // HD 2500/4000) — turning both down here is the single biggest lag
  // reduction available on that class of hardware, well before touching
  ...
  // UnrealBloomPass is the most expensive single thing in this scene — it
  // renders and blurs several extra full-screen passes. Its resolution
  // argument controls the internal render-target size *independently* of
  // the actual canvas size, so halving it here keeps the same visual
  // bloom look at a quarter of the pixel cost, which matters a lot on
  ...
  // ——— COLORS ———
  const C_BRIGHT = 0x38bdf8;
  const C_MID = 0x0284c7;
  const C_DIM = 0x0c4a6e;
  const C_FAINT = 0x082f49;
  const C_HOT = 0x7dd3fc;
  ...
  function makeTextSprite(text: string, size = 0.08) {
    ...
    ctx.fillStyle = `rgba(${(70 + Math.random() * 60) | 0}, ${(170 + Math.random() * 60) | 0}, 255, ${alpha})`;
  ...
  // Each floating "code" glyph is its own Sprite with its own texture and
  // draw call — Three.js doesn't auto-batch sprites with different
  // textures, so 1200+100+400 = 1700 of these was 1700 draw calls every
  // frame, on top of everything else in the scene. That's the single
  // costliest part of this scene on weaker/integrated GPUs, more than the
  // bloom pass. Cut roughly in half below — still reads as "dense code
  // coverage" at a normal viewing distance, at less than half the cost.
  ...
  // ═══════════════════════════════════════════════
  // 2000 individually-updated dust particles is a lot of per-frame CPU
  // work updating their positions before the GPU even sees them. 600
  // keeps the "drifting dust" feel with a third of the update cost.
  const dustCount = 600;
  ...
  g.addColorStop(0.2, "rgba(14,165,233,0.6)");
  g.addColorStop(0.5, "rgba(2,90,180,0.15)");
